...
%{
	...
	void print(int, int, std::string, int);
	void printFunc(int, int, std::string, int);
...
%type <str> NAME
%type <complexity> funcdef opt_FINALLY except_clause plus_except EXCEPT compound_stmt simple_stmt suite stmt if_stmt while_stmt for_stmt star_ELIF plus_stmt try_stmt classdef opt_ELSE with_stmt decorated import_as_name opt_DOUBLESTAR_NAME dotted_as_name dotted_name star_COMMA_NAME
...
decorated // Used in: compound_stmt
	: decorators {is_Decorated = 1;} classdef
	{ $$ = $3; is_Decorated = 0;}
	| decorators {is_Decorated = 1;} funcdef
	{ $$ = $3; is_Decorated = 0;}
	;
funcdef // Used in: decorated, compound_stmt
	: DEF NAME {func_Level++;} parameters COLON suite
	{   
          $$ = $6+1;
	  func_Level--;
          if(func_Level == 0)
	  {
		if(is_Decorated == 0)
			printFunc(@1.first_line,@1.first_column,$2,$$);
		else
			printFunc(@1.first_line-1,@1.first_column,$2,$$);
...
compound_stmt // Used in: stmt
	: if_stmt 
	{ $$ = $1; print(@1.first_line,@1.first_column,"If",$$+1);}
	| while_stmt  
	{ $$ = $1; print(@1.first_line,@1.first_column,"Loop",$$+1);}
	| for_stmt 
	{ $$ = $1; print(@1.first_line,@1.first_column,"Loop",$$+1);}
	| try_stmt 
	{ $$ = $1; print(@1.first_line,@1.first_column,"TryExcept",$$+1);}
	| with_stmt 
	{ $$ = $1;}
	| funcdef 
	{ $$ = $1;} 
...
classdef // Used in: decorated, compound_stmt
	: CLASS NAME {stk.push($2); if((@1.first_column) == 1) class_Flag = 1;} LPAR opt_testlist RPAR COLON suite
	{  $$ = $8;
           stk.pop();
           if(stk.empty())  class_Flag = 0;
	   delete [] $2;
	}
	| CLASS NAME {stk.push($2); if((@1.first_column) == 1) class_Flag = 1;} COLON suite
	{  $$ = $5; 
           stk.pop();
           if(stk.empty())  class_Flag = 0;
	   delete [] $2;
...
void print(int lineno, int columnno, std::string str, int complexity)
{
	if((columnno == 1))
	{
		std::cout<<"('"<< str <<" "<< lineno <<"', "<< complexity <<")"<<std::endl;
	}
}

void printFunc(int lineno, int columnno, std::string Funcname, int complexity)
{
	std::string str;
        std::stack<std::string> cname = stk;
        if(columnno == 1)
	{	std::cout<<"(\"" << lineno <<":"<< columnno-1 <<": '";   
	  	std::cout<< Funcname <<"'\", "<< complexity <<")"<<std::endl;
        }
        if(class_Flag == 1 && !stk.empty())
        {
                while(!cname.empty())
                {
     	  		str = cname.top()+"."+str;
                ...
                std::cout<<"(\"" << lineno <<":"<< columnno-1 <<": '";
	        std::cout<< str << Funcname <<"'\", "<< complexity <<")"<<std::endl;
          }
}
